...
Function TOMB-Event($Computer, $Path, $LogID) {
    ...
    Catch [System.Net.NetworkInformation.PingException] { "$(Get-Date): Host ${Computer} Status unreachable." | Out-File -FilePath $Path\logs\ErrorLog\windowslogs.log -Append }
    Catch [System.Management.Automation.Remoting.PSRemotingTransportException] { "$(Get-Date): Host ${Computer} Access Denied" | Out-File -FilePath $Path\logs\ErrorLog\windowslogs.log -Append }
    If ($connectionCheck){EventCollect($Computer)}
    Else { "$(Get-Date) : $($Error[0])" | Out-File -FilePath $Path\logs\ErrorLog\windowslog.log -Append }

}

Function EventCollect($Computer, $LogID){
    If ($null -eq $LogID) { $LogIDs = $(Get-content $Path\includes\EventIDs.txt | Where {$_ -notmatch "^#"}) }
    Foreach ($LogID in $LogIDs) { [string[]]$LogIDx += $LogID -Split("`t") | Select -First 1 }
    Foreach ($Log in $LogIDx) {
        ...
        $EventLog = "Get-WmiObject Win32_NTLogEvent -Filter 'EventCode=$Log and (RecordNumber > $LastRun)' -ErrorAction Stop"
        ...
        Try { $EventLogFinal
            ...
            If ($EventLogFinal -ne $null){ 
                Foreach($obj in $EventLogFinal){ $obj  | TOMB-Json | Out-File -FilePath $Path\Files2Forward\\Events\${Computer}_${Log}_logs.json -Append -Encoding UTF8
            $EventLogFinal.RecordNumber[0]  | Out-File -FilePath $Path\modules\DO_NOT_DELETE\${Computer}_${Log}_timestamp.log } }
            Else { "$(Get-Date) : $($Message)" | Out-File -FilePath $Path\logs\ErrorLog\windowslogs.log -Append} }
        #Any exception messages that were generated due to error are placed in the Errorlog: Windowslogs.log
        Catch { "$(Get-Date): $($Error[0])" | Out-File -FilePath $Path\logs\ErrorLog\windowslogs.log }
    }
}
