...
Function TOMB-Service($Computer, $Path){
    ...
    Catch [System.Net.NetworkInformation.PingException] { "$(Get-Date): Host ${Computer} Status unreachable." | Out-File -FilePath $Path\logs\ErrorLog\service.log -Append }
    Catch [System.Management.Automation.Remoting.PSRemotingTransportException] { "$(Get-Date): Host ${Computer} Access Denied" | Out-File -FilePath $Path\logs\ErrorLog\service.log -Append }
    If ($connectionCheck){ServiceCollect($Computer)}
    Else { "$(Get-Date) : $($Error[0])" | Out-File -FilePath $Path\logs\ErrorLog\service.log -Append }
}

Function ServiceCollect($Computer) { 
    #Generation of the scriptblock and allows remote machine to read variables being passed.
    $Service = "Get-WmiObject -Class 'Win32_Service' -ComputerName $Computer -Property * -ErrorAction Stop"
    ...
    Try { $Service_List
        ...
        Foreach($obj in $Service_List){ $obj | TOMB-Json | Out-File -FilePath $Path\Files2Forward\Service\${Computer}_service.json -Append -Encoding utf8 } }
        Else { "$(Get-Date) : $($Message)" | Out-File -FilePath $Path\logs\ErrorLog\service.log -Append} }
    Catch [System.Net.NetworkInformation.PingException] { "$(Get-Date): Host ${Computer} Status unreachable after." | Out-File -FilePath $Path\logs\ErrorLog\service.log }
}
