...
#Variable storage
#$DCIP = "10.0.0.100"
#$ADComputers = Get-AdComputer -Filter * -Properties Name,DistinguishedName -Server $DCIP
...
Function Main { 
    ...
    if($Domain -and $Server){
        $Domain_Computers = $( Get-ADComputer -Filter * -Properties Name,DistinguishedName -Server $DCIP -SearchBase $Domain | Select Name )
    ...
    if($Domain -and ($Server -eq "")){ 
        $Domain_Computers = $( Get-ADComputer -Filter * -Properties Name,DistinguishedName -SearchBase $Domain | Select Name )
            Foreach ($Hostx in $Domain_Computers){ ( $Hostx -replace "@{Name=","" ) -replace "}","" | Out-File -FilePath .\includes\tmp\DomainList.txt -Append}
            Collects
    }
    If($Computer){
...
Function Collects {
    If($Collects){
        Foreach ($obj in $Collects){
            Write-Host "Collection is: $obj"
            Write-Host "Host is: $Computer"
            #.$obj([System.Array]$Computer, [System.Array]$LogID)
